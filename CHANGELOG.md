...
### Added
- **SQS queue-level redrive**: `RedrivePolicy` (`deadLetterTargetArn` + `maxReceiveCount`) is parsed from the template and now **enforced** by the self engine — a message already delivered `maxReceiveCount` times moves to the dead-letter queue instead of being redelivered forever and vanishing at retention. The moved message keeps its `MessageId`, body, message attributes and the `MD5OfMessageBody`/`MD5OfMessageAttributes` digests, so a DLQ consumer sees the same message the source queue handed out. FIFO message groups are unblocked when the poison message moves — the rest of the partition drains instead of stalling behind it head-of-line. A missing or unresolvable dead-letter queue warns **once** and leaves redrive off for that queue (non-fatal, like every other provisioning step). `RedriveAllowPolicy` (stored and returned, never enforced) and the manual redrive API (`StartMessageMoveTask`/`ListMessageMoveTasks`/`CancelMessageMoveTask`) remain out of scope — drain a DLQ with ReceiveMessage/SendMessage.

### Fixed
- **Raw ApiGatewayV2 routes now work with the shape the Serverless Framework actually emits.** 0.16.0's assembler handled a hand-authored idiom, so real projects saw `no ::Integration for Target "(none)"` and their raw routes silently disappeared. All of it reduces now: `Target: !Sub "integrations/${LogicalId}"` in both the string and the `[template, vars]` forms; `AuthorizerUri`/`IntegrationUri` arriving in the API Gateway *invocation-URI* wrapper (`arn:aws:apigateway:…:lambda:path/2015-03-31/functions/<lambdaArn>/invocations`) are unwrapped to the inner Lambda ARN through a new shared `src/server/services/lambda-invoke-uri.ts` — the invoke listener's own route and this unwrap are now generated from one pattern and cannot drift apart; `Ref` resolves the AWS pseudo-parameters (`{Ref: AWS::Partition}`/`{Ref: AWS::Region}`/`{Ref: AWS::AccountId}` inside the framework's own compiled `Fn::Join` previously reduced to a garbage ARN); `AWS::Lambda::Permission.FunctionName` given as `{Ref: LogicalId}` — a **name**, not an ARN — now matches an ARN-targeted route instead of emitting a false "no matching permission" warning; and `${HttpApi}` resolves to the framework's own `::Api`. The regression fixture is a **real `serverless package` snapshot** carrying the framework's compiled `::Route` mirrors *and* hand-written `!Sub` resources on the same `HttpApi`, so the `(method, normalized path)` de-duplication against `serverless-state.json` is exercised for real rather than assumed.

## [0.16.0] - 2026-07-16

Three things a template could declare but LSS quietly dropped — secrets, bucket CORS and raw `AWS::ApiGatewayV2::*` routes — are provisioned on registration now, so a service's first boot is already its working boot instead of the one that needs a bootstrap script next to it. The event-source-mapping path stops improvising in the same release: `FilterCriteria` is enforced instead of merely stored and echoed, `maximumRetryAttempts` follows the AWS default (retry until the record ages out of the stream) instead of a home-grown cap of five, and `ReportBatchItemFailures` is honored on DynamoDB streams and SQS alike.

### Added
- **Secrets Manager provisioned on boot**: `AWS::SecretsManager::Secret` is now parsed from the template and created at service registration — `SecretString` verbatim, or `GenerateSecretString` synthesized through the emulator's own `GetRandomPassword` with the real AWS defaults (`PasswordLength` 32, every character class included, `RequireEachIncludedType` true), the generated value injected into `SecretStringTemplate` at `GenerateStringKey` when both are present, exactly like CloudFormation expands it (`Description`, `KmsKeyId` and `Tags` come along; a secret declaring neither value warns and is created empty, which is what AWS does). Plus a **boot seed path** for secrets no template declares: `seeds/secrets/<name>.json` files (walked recursively — nested directories map to `/`-separated secret names like `billing/receipt-signing-key`) merged with an optional `secrets:` map in `lss.config.json` (the config map wins a name collision, with a warning). A seed value is either the `SecretString` itself (a bare string, or a bare JSON object that *is* the payload — a DB-credential blob just works) or a descriptor carrying `secretString`/`generateSecretString` plus `description`/`kmsKeyId`/`tags`. Seeds are applied **before cached services are reactivated**, so a handler's first `GetSecretValue` — including one fired by a relay during rehydrate — already finds an `AWSCURRENT` version. Both paths are idempotent (an existing active secret is skipped, never clobbered; one scheduled for deletion is warned and skipped) and non-fatal (a failure warns, boot continues), and they share one value resolver (`src/server/services/secret-value.ts`) so the CFN and seed expansions cannot drift apart. **This closes the "Known gaps" item recorded under 0.11.0**: declare-and-forget secrets in `resources:` no longer need a bootstrap `CreateSecret`.
- **S3 `CorsConfiguration` from CloudFormation**: `parseS3` now reads `CorsConfiguration.CorsRules[]` — `AllowedOrigins`/`AllowedMethods`/`AllowedHeaders`, `ExposedHeaders` (the CFN spelling) or `ExposeHeaders` (the S3 wire member), `MaxAge` or `MaxAgeSeconds` with a literal `0` preserved rather than dropped as falsy, and `Id` — skipping a rule that declares no methods or no origins, both of which AWS requires. The provisioner applies the result with `PutBucketCors` when the bucket is created, so a browser preflight against the bucket succeeds on the **first** boot with no separate bootstrap step; `PutBucketCors` is a full-replace PUT, so re-registration re-sends the same configuration and is naturally idempotent, and the call is non-fatal like the versioning pass beside it. The engine could already serve CORS since 0.13.0 — it just had no way to learn the bucket's rules from the template.
- **`FilterCriteria` enforced on event source mappings**: stream and SQS mappings previously *stored* `FilterCriteria` and echoed it back from the control plane while delivering every record anyway. It is now compiled into a predicate (`src/server/engine/dispatch/filter-criteria.ts`) and applied before invoking: each `Filters[].Pattern` is a JSON-encoded EventBridge-style content filter, patterns are OR'd and sibling keys inside one pattern AND'd, reusing the engine's own pattern matcher; an absent/empty `FilterCriteria` compiles to always-true (the pre-filtering default), and the control plane validates at write time (non-object `Filters`, more than the AWS limit of 5 patterns, an unparseable `Pattern`, an unsupported operator → `InvalidArgumentException`). Filtered-out **stream** records still advance the cursor, so they are dropped once and never reappear; filtered-out **SQS** messages are removed from the batch before the handler sees it.
- **Raw `AWS::ApiGatewayV2::Api`/`::Route`/`::Integration`/`::Authorizer` provisioning** (plus the advisory `AWS::Lambda::Permission` grant): routes declared directly under CFN `resources:` register into the same route registry `httpApi:` events feed, including **cross-stack ARN resolution** — a route whose integration or authorizer targets a Lambda owned by *another* registered service resolves through the global registry. Raw routes are de-duplicated against the routes that come from `serverless-state.json` by `(method, normalized path)`, so a template that declares both shapes binds one listener, not two.

### Fixed
- **`maximumRetryAttempts` now means what AWS means**: the hard cap of 5 is gone. Omitted or `-1` retries the failing batch **until the record ages out of the stream** (the AWS default) rather than giving up after five attempts, and an explicit `-1` no longer produced **zero** invocations — the batch was previously dropped without the handler ever running once. A positive `N` still means N retries; exhausting the budget (or aging out) logs which condition ended it (`RetryAttemptsExhausted` / `RecordAgeExpired`) and forwards the still-failing records to the `OnFailure` destination as before.
- **`ReportBatchItemFailures` is honored on both stream and SQS mappings**. DynamoDB streams: the tailer checkpoints just before the **earliest** still-failing record (records are ascending, so everything from that index on is retried — unlisted records after it included), only the still-failing suffix reaches the DLQ/`OnFailure` destination, an empty `batchItemFailures` list commits the whole batch, and a malformed response (non-array, empty/unknown `itemIdentifier`) retries the batch exactly as a thrown handler would; the attempt budget is *not* reset when the suffix shrinks. SQS: only the receipt handles **not** reported as failures are deleted, so the reported ones redeliver on their visibility timeout instead of the whole batch being dropped or the whole batch being redelivered.
...
### Added
- **Secrets Manager emulator** (`src/server/engine/emulators/secretsmanager/`), served by the self engine over AWS JSON 1.1 under the `secretsmanager` SigV4 scope / `X-Amz-Target`: `CreateSecret`, `GetSecretValue`, `PutSecretValue`, `UpdateSecret`, `DescribeSecret`, `DeleteSecret` (recovery window or `ForceDeleteWithoutRecovery`), `RestoreSecret`, `ListSecrets`, `TagResource`/`UntagResource`, `GetRandomPassword`. Real version staging — a new value promotes to `AWSCURRENT` and demotes the old holder to `AWSPREVIOUS`, and `GetSecretValue` resolves by `VersionStage` or `VersionId`; `SecretString` and `SecretBinary` both round-trip; `ClientRequestToken` gives idempotent create/put; secrets are scoped per region and persist in a catalog (not encrypted locally — `KmsKeyId` is accepted and ignored). Errors carry the names the SDK branches on (`ResourceNotFoundException`, `ResourceExistsException`, `InvalidParameterException`, `InvalidRequestException`). This retires the LocalStack Secrets Manager the self engine previously reached only by proxying through `fallbackEndpoint`. Registered in the router (`secretsmanager` → JSON 1.1), the `/_localstack/health` alias and `healthDetail().services`.
- **S3 presigned POST** (browser form upload): the S3 emulator now handles `POST /<bucket>` with `multipart/form-data` — the `key` field (with `${filename}` substitution), `Content-Type`, `success_action_status` (200/201-with-`PostResponse`/204) and `success_action_redirect` (303 with `bucket`/`key`/`etag` appended), plus `x-amz-meta-*` fields as object metadata. The upload stores the object exactly like `PutObject` (content-addressed blob, index entry) and emits an `s3:ObjectCreated:Post` event, so bucket-notification Lambdas fire. Backed by a new dependency-free, byte-accurate multipart parser (`src/server/engine/emulators/s3/multipart.ts`). The POST policy/signature fields are accepted but not verified — the engine never verifies SigV4 either.
- **Secrets Manager tab in the dashboard**: the nav gains a **Secrets** section — a list (name, description, version count, last changed, "deletion scheduled" badge) with a per-secret inspector (ARN, tags, `AWSCURRENT`/`AWSPREVIOUS` staging labels per version) and an explicit **Reveal value** button (a plain list never carries secret material — the value is a separate `GET /api/secrets/:name/value` call). Backed by new `GET /api/secrets`, `GET /api/secrets/:name`, `GET /api/secrets/:name/value` routes and a `SecretsExplorer` service that targets the active engine over the AWS wire (works on the self engine and LocalStack alike); the region selector is honored. New runtime dependency: `@aws-sdk/client-secrets-manager`.
- **`examples/self-hosted` now showcases both features on the self engine**: `billing-service` gains `GET /attachments/upload-url` (returns a `createPresignedPost` form the browser uploads straight to S3) and `GET /receipts/{id}/signature` (reads an HMAC signing key from Secrets Manager — lazily created via `CreateSecret`/`GetRandomPassword`, read via `GetSecretValue` — to sign a receipt id, the pattern of a service reading a signing key at request time). README documents both flows; the secret appears in the dashboard's Secrets tab.
- **Full-stack wire conformance tests** (`tests/unit/engine/wire-secretsmanager-s3post.test.ts`): boot the real `SelfEngineBackend` HTTP front door and drive both features exactly as the AWS SDK / a browser would — the integration layer no unit test previously covered, which is the class of gap that let earlier omissions reach users. Plus focused unit suites for the Secrets Manager emulator, the S3 POST handler, the multipart parser, and the `SecretsExplorer` + `/api/secrets` route.

### Known gaps (tracked follow-ups)
- **CloudFormation `AWS::SecretsManager::Secret` is not yet provisioned on registration** — declare-and-forget secrets in `resources:` are not created; create them via the SDK (`CreateSecret`, e.g. in a bootstrap step) for now. Wiring the provisioner is a follow-up.
