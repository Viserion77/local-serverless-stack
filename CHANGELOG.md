...
- **Full-stack wire conformance tests** (`tests/unit/engine/wire-secretsmanager-s3post.test.ts`): boot the real `SelfEngineBackend` HTTP front door and drive both features exactly as the AWS SDK / a browser would — the integration layer no unit test previously covered, which is the class of gap that let earlier omissions reach users. Plus focused unit suites for the Secrets Manager emulator, the S3 POST handler and the multipart parser.

### Known gaps (tracked follow-ups)
- **CloudFormation `AWS::SecretsManager::Secret` is not yet provisioned on registration** — declare-and-forget secrets in `resources:` are not created; create them via the SDK (`CreateSecret`, e.g. in a bootstrap step) for now. Wiring the provisioner (and its `@aws-sdk/client-secrets-manager` dependency) is a follow-up.
- **Examples don't yet exercise transactions, Secrets Manager or presigned POST.** The examples are demos, not the conformance surface — the unit + wire tests above are. Extending an example (the `self-hosted` one, the "drop LocalStack" target) to demonstrate these is a follow-up.
