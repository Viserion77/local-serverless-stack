...
- **VS Code tasks for `examples/localstack-ultimate`** (`.vscode/tasks.json`): setup, start (Docker + `LOCALSTACK_AUTH_TOKEN`), offline, seed, drive demo and stop — same style as the existing example task sections, which previously covered only `self-hosted` and `localstack-free`.
...
- The root package version is now `0.10.0`. The `serverless-lss` plugin package was not changed.
...
- Example service lockfiles synced to the `serverless-lss` plugin's current optional `serverless >=3.40.0` peer dependency (they still pinned the removed `osls` peer).
