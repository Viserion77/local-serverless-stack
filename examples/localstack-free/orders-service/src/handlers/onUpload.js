...
// records it in orders-Orders to prove the round-trip works end-to-end (the
// write also flows through the table stream → onOrderStream → SNS).
exports.handler = async (event) => {
  ...
  for (const record of records) {
    ...
    // Stash an entry in orders-Orders so the trigger is observable from the
    // dashboard's DynamoDB explorer without adding a dedicated table.
    await doc.send(new PutCommand({
      TableName: process.env.ORDERS_TABLE,
      Item: {
        id: `upload:${bucket}/${key}`,
        item: key,
        user: 's3-upload',
        status: 'uploaded',
        size,
