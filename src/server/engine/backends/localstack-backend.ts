// LocalStack lifecycle backend (docker container, managed/external). This is
// the code that used to live in services/localstack-manager.ts — that module
// is now a thin facade over EngineManager so existing call sites keep working
...
export class LocalStackBackend implements EngineBackend {
  readonly kind = 'localstack' as const;
  ...
  healthDetail(): Record<string, unknown> {
    return {
      ...
      image: this.configManager.isExternal() ? undefined : this.configManager.getLocalStackImage(),
    };
  }
}
