...
problem it solves). 89 features in nine areas — start with [Quick Start](#quick-start) if you just
want it running.
...
**How it works.** `start` checks the PID file first: if the recorded PID answers `kill(pid, 0)` it just reprints the URLs and returns; a dead PID file is deleted and boot continues. It resolves `dist/server/index.js`, opens the log file in append mode and `spawn`s `node` detached with stdio piped to that file, `unref`s the child and writes its PID. Ports come from config plus env (`serverPort` and the self engine both default to **14566**; the DynamoDB proxy is off, port **8000** when enabled), and they are passed down as `PORT`, `LSS_ENABLE_DYNAMO_PROXY`, `LSS_DYNAMO_PROXY_PORT`. Two seconds later it re-probes the PID: alive prints "Service is running", **dead prints the failure, the tail of the log, who holds the port — and exits 1**, because a daemonized boot discovers a busy port only after the CLI has printed a PID, and a step that fails must not report success. `lss start --wait[=<seconds>]` (default 30) goes further and blocks until `GET /api/health` answers, which is what a sequential chain needs so the next `register` isn't run against a stack that never came up. `stop` sends `SIGTERM` and then polls `kill(pid, 0)` every **200 ms for up to 10 s** before deleting the PID file — without that wait an immediate `lss start` would race the dying process for the port and die with `EADDRINUSE`. `status` prints PID, server URL, self engine URL, proxy URL (only when enabled) and log path, and self-heals a stale PID file. When there is **no** PID file for this project it asks `GET /api/config` who is on the port and reports `port 14566 is in use by the orchestrator of /other/project` instead of `NOT RUNNING` — under `--net=host` the port belongs to the whole machine, and a status that says "not running" about a port answering 200 sends you looking in the wrong place.

**Where it lives.** `bin/cli.js` — `startOrchestrator()`, `waitForHealthy()`, `reportStartFailure()`, `probePortOwner()`, `stopOrchestrator()`, `waitForExit()`, `showStatus()`, `runtimePaths()`. `src/server/index.ts` — the orchestrator entry point that gets spawned.

**Why it exists.** One command replaces "start LocalStack, then start one `serverless-offline` per service". Because the process is detached and PID-tracked, the stack survives closing the terminal, and `status` answers the "is it up?" question — and, when the answer is no, the "then who has my port?" question — without grepping `ps`.
...
**How it works.** It requires a running orchestrator (PID file present, otherwise it prints the start hint and exits 1), then resolves each positional argument to an absolute path — defaulting to `.` when none is given — and skips (counting as failed) any directory that does not exist. For each path it `POST`s `{ servicePath, repackage }` to `/api/services/register` on the server port; the orchestrator does everything else: packaging on demand via `autoPackage`, then reading name, region and `custom.lss` ports out of the packaged `serverless-state.json`. Each success prints resource/function/route counts plus any warnings the server returned — including everything provisioning could **not** do (a resource that failed, a rule waiting for a bus, HTTP routes with no `apiPort`), which used to reach the daemon log only; if any target failed, the process exits 1.

**`--repackage`, and the warning that made it necessary.** `autoPackage` packages only when the template is **missing**, so registering an already-packaged service re-reads the artifacts on disk — which may be many commits old. The command answers `✓` in a fraction of a second either way, and since registering is the natural gesture after editing code, that `✓` reads as "my code is loaded" when it can mean "yesterday's build is loaded". Two changes: registration now compares the newest artifact in `.serverless/` against the service's sources (skipping `node_modules`, `dist`, `build`, `coverage` and dot-directories) and emits a warning naming the file that moved, both timestamps and the service's own package command; and `--repackage` runs that command before reading, regardless of `autoPackage` — an explicit operator gesture, not an automatic one. The warning never blocks the registration: what was packaged is still registered, it just stops being silent about its age.
...
**How it works.** It calls `GET /api/services/scan`, which walks the project root (skipping `node_modules`, `.git`, `.serverless`, `.lss`, `.build`, `.esbuild`, plus any directory listed in `scanIgnore`) and returns one row per service. Each row prints three flags — `registered` (known to this orchestrator), `installed` (a `node_modules` is resolvable from the service dir upward to the root, so packaging can run without an install) and `packaged` (`.serverless/cloudformation-template-update-stack.json` exists) — followed by the resolved `api:`/`invoke:` ports when known. A second line shows the effective package command for that service (per-service `servicePackaging`, else the global one), and any scan warnings are listed underneath — the "not packaged" one worded from the **effective** `autoPackage`, so it never promises packaging a config with `autoPackage: false` will refuse. Empty results print the root that was searched.
...
**How it works.** When the CloudFormation template is missing and `autoPackage` is on (default **false**; `LSS_AUTO_PACKAGE` also sets it), the registrar runs the effective package command with the service directory as cwd — default `npx serverless package`, global `packageArgs` then any per-service `packageArgs` appended as discrete argv (no shell re-parsing), `packageEnv` merged over `process.env`, timeout `300000` ms with `SIGTERM` then `SIGKILL` 2 s later — and re-reads the template. A non-zero exit or a timeout raises `ServerlessPackageError`; the registrar logs the full stdout/stderr and answers 500 pointing at `/tmp/lss-orchestrator.log`. With `autoPackage` off, a missing template is a 400 telling you to run `serverless package`. `POST /api/services/package` runs the same effective command on demand and returns `exitCode`, `durationMs` and the last 8000 characters of output (422 on failure). The source watcher's full reload re-packages the same way before re-registering. Because the trigger is a **missing** template, `autoPackage` never refreshes a service that was packaged once — that is what `lss register --repackage` (`{ repackage: true }` on the API) is for, and what the stale-artifact warning reports when neither ran.

**What may be run.** `packageCommand` is settable through `PUT /api/config`, which has no authentication, and it becomes `spawn(firstToken, restTokens)` with `packageArgs` appended — so validating it as "any non-empty string" made the `/start` and `/install` allowlists pointless, since two calls reached the same `spawn()`. Checking only the **first token** does not close it either, and that version of the fence was walked straight through: every package manager is itself an interpreter one subcommand in, and `spawn('npm', ['exec','-c','<shell string>'])` runs that string with no `shell: true` anywhere — as do `npx -c`, `yarn dlx`, `pnpm dlx`, `yarn node -e` and `npm i <pkg>` (install scripts). So what a command written through that endpoint must match — global or per-service — is a **grammar over the whole argv**, tokenized exactly the way the packager tokenizes it before `spawn()`:
...
`npx serverless package` (the default), `npm run package`, `yarn run package`, `sls package -c custom.yml` and `npx -y serverless@3.38.0 package` all pass. `/bin/sh -c …`, `node -e …`, `/usr/bin/env`, `./tool`, `npm exec -c …`, `npx -c …`, `yarn dlx …` and a bare `yarn package` (yarn 1's implicit `run`, which is what makes `yarn node -e` indistinguishable from a script name) are 400s. On top of the grammar, every token of the command **and every `packageArgs` element** is screened for flags that re-point an allowed program somewhere else — `--node-options`, `--script-shell`, `--shell`, `--call`, `--userconfig`, `--globalconfig`, `--use-yarnrc`, `--registry` (npx fetches the CLI when the service has no local copy, and npm reads its config flags from anywhere in the argv), `--prefix`, `--cwd`, `--dir` and node's own `--require`/`--eval`/`--print`/`--import`/`--loader`/`--input-type` — compared with dashes and underscores stripped, so `--nodeOptions` is the same flag. Serverless's short flags (`-c`, `-p`, `-r` = `--config`, `--package`, `--region`) are deliberately *not* screened: the grammar already makes them unreachable as npm/npx options. `packageEnv` likewise refuses keys the runtime or dynamic linker reads before `main()` — `NODE_OPTIONS`, `NODE_REPL_EXTERNAL_MODULE`, `LD_PRELOAD`, `LD_AUDIT`, `LD_LIBRARY_PATH`, `DYLD_INSERT_LIBRARIES`, `DYLD_LIBRARY_PATH`, plus `PATH` and `NODE_PATH` (the child's `PATH` is what `execvp(3)` uses to pick which file named `npm` runs, so setting it names an arbitrary binary), compared case-insensitively — since `{"NODE_OPTIONS":"--require /tmp/x.js"}` turns an otherwise blameless `npx serverless package` into an arbitrary-code runner whichever runner passed. Two limits worth stating out loud: the fence is on the API and **not** the read path (a hand-edited `lss.config.json` and `LSS_PACKAGE_COMMAND` are the operator's own shell, and anyone who can write either already runs code here), and what stays expressible is the project's own build — `npm run <script>` runs whatever the service's `package.json` declares and `serverless package` reads its `serverless.yml`, plugins included. That is code the operator already trusts by pointing LSS at the directory, and forbidding it would mean the dashboard could not set a package command at all, which is what onboarding is built on. What is gone is the part that made the other allowlists worthless: through this API a caller can no longer choose the program, only ask the project's own build to run.
...
<details>
<summary><b>Guarded command surfaces</b> — install, package and start are the only commands the API can run on your host, and each is narrowed to exactly that one</summary>

**How it works.** `POST /api/services/install` runs a package-manager install in a scanned service (packaging fails without `node_modules`). The command defaults to `npm install`, and validation is on its *shape*: the first token must be `npm`, `yarn` or `pnpm`; the second must be `install`, `ci`, `i` or `add` (`yarn`/`pnpm` may omit it, `npm` may not); every remaining token must be a flag **on an allowlist** — `--production`, the three lockfile-strictness spellings (`--frozen-lockfile`, `--no-frozen-lockfile`, `--immutable`), `--no-audit`, `--no-fund`, `--prefer-offline`, `--offline`, `--legacy-peer-deps`, `--force`, `--silent`, `--quiet`, plus `--omit`, `--include` and `--loglevel` which may carry a value in the `--flag=value` spelling. An allowlist rather than "must start with `-`", because a flag is not inert and the dangerous ones look exactly like the harmless ones: `--registry=http://…` decides *where the installed code comes from* and its `postinstall` then runs on your host, `--userconfig=` swaps the whole npmrc, and `--prefix=` / `yarn --cwd=` / `pnpm --dir=` relocate the install out of the very directory the path fence vetted. So `npm install lodash`, `npm exec -- anything`, `node -e "…"` and `npm install --registry=…` are all unrepresentable and answered 400. The path must be absolute and free of `..`, and both it and the project root are **realpath-resolved before they are compared** — a lexical check passes a symlink that sits inside the root but points outside it (`services/orders → /etc`), and `statSync` and the spawn would follow it — then it must be an existing directory (a `statSync` throw is treated as not-a-directory rather than crashing the handler). The resolved real path is what the command runs in. Timeout is the service's effective `packageTimeoutMs` (default 300000). The response carries `exitCode`, `durationMs` and the last 8000 characters of stdout+stderr; a non-zero exit is 422. `POST /package` shares the same path guard.

**Where it lives.** `src/server/routes/services.ts` — `resolveServiceDir()` (realpath project-root confinement), `installCommandError()` with `INSTALL_ALLOWED_FLAGS`/`INSTALL_ALLOWED_VALUE_FLAGS` (command shape), `POST /install`, and `START_RUNNERS`/`STAGE_PATTERN` for `POST /:name/start`. `src/server/services/serverless-packager.ts` — the spawn/timeout/output capture. `src/server/services/process-manager.ts` — the `spawn` behind a start. `src/ui/src/pages/OnboardingPage.vue` — the install button in the onboarding checklist.

**Why it exists.** The dashboard onboarding walks install → package → register for a service you just cloned, and every legitimate caller gets its paths from `GET /scan`, which never looks outside the project root. Without the fence and the command allowlist, an endpoint on a local port would be arbitrary command execution on the host — which is not a hypothetical: `POST /api/services/:name/start` took `command`, `args`, `cwd` and `env` straight from the request body through **0.17.2**, with `node` on its allowlist, and was exactly that. It now derives argv, cwd and env from the server — the service's recorded root plus `npm run start`/`start:<stage>` — and the two values a caller may still send are narrowed: `command` to `npm`/`yarn`/`pnpm` (`node` and `npx` are off the list) and `stage` to a `[A-Za-z0-9._-]+` script-name token, so it can pick a script of the service's own `package.json` and nothing else. The stack's three command surfaces — install, package, start — are each narrowed to the one command they need.
...
**Who can reach it.** That listener binds **`127.0.0.1` by default** — the API has no authentication and every table, queue, bucket and secret in the stack is readable through it, so binding every interface (which is what a bare `listen(port)` does, and what LSS did through 0.17.2) offers all of that to anything that can route to the port. The bind is a property of the **process**, not of this one file: `LSS_BIND_HOST` is resolved once in `src/server/services/bind-host.ts` and read by *every* listener LSS opens — this one, each registered service's API Gateway port and Lambda Invoke API port (`gateway-manager.ts`), the engine's own front door when `serverPort` and `selfEngine.port` differ (`self-backend.ts`), and the optional DynamoDB proxy (`dev/dynamo-proxy.ts`). Through 0.17.2 those last three ignored it: a bare `server.listen(port)` binds the `::` wildcard, so the invoke ports — which run a registered handler with the request body as its event, on a raw `http.Server` that Express and therefore CORS never see — answered on the LAN even with the dashboard fenced to loopback, and a split `selfEngine.port` published the entire AWS data plane the same way. One value now covers the process, which is what makes widening a single decision instead of four.

**Opening it on purpose.** `LSS_BIND_HOST=0.0.0.0 LSS_CORS_ORIGINS='*' lss start` is the documented opt-in, and both halves are usually needed together, because the layout that needs them is *LSS in a container, browser and the developer's own frontends on the host*. The bind is what makes a published port work at all (`-p 14566:14566` forwards to the container's external interface, which a loopback socket refuses); the origin list is what lets a page that is not on `localhost` read the answer. With `LSS_CORS_ORIGINS` unset, CORS is granted only to `http://localhost`, `http://127.0.0.1` and `http://[::1]` on any port — the Vite dev dashboard on `3101` and nothing else, since the built dashboard is served by this same process and is therefore same-origin. Set it to a comma-separated list of exact origins (`http://192.168.1.20:5173`) to name your own frontends, or to `*` when they are unpredictable; setting it *replaces* the loopback default rather than extending it. A request with no `Origin` (curl, the CLI, `LssClient`, the MCP server, an SDK) is not a cross-origin browser request and is unaffected, and the AWS wire never sees the middleware at all: `isAwsRequest()` hands SDK traffic to the engine before Express runs. Both variables are environment-only — no `lss.config.json` key, no Settings field — precisely so that widening the boundary cannot be done through the API the boundary protects; VS Code devcontainer forwarding needs neither, since it attaches from inside the container where loopback is the same network stack. Widening is never silent: a non-loopback bind or a `*` origin list prints a boot warning naming exactly what it opened and how to undo it, and an address that is not this host's fails with an `EADDRNOTAVAIL` message pointing at the variable instead of reading as a port conflict. What you accept is an unauthenticated API whose callers can read and write your local emulator data and read secret values — bounded rather than a shell on the host, because `/start` derives its argv server-side, `/install` is shape- and flag-allowlisted, and `packageCommand`/`packageArgs` must match the packaging grammar. Bounded is not zero: a reachable caller can still ask the project's own build to run, so "a network you trust" is a precondition and not a formality.

**Where it lives.** `src/server/engine/http/is-aws-request.ts` — the one-directional AWS/browser demux. `src/server/engine/http/router.ts` — service resolution, dispatch, protocol-correct success/error framing, health endpoints. `src/server/engine/http/sigv4.ts` — credential-scope parsing (`{service, region}` only). `src/server/services/bind-host.ts` — the process-wide network posture: `getBindHost()`, `isLoopbackBind()`, `isOriginAllowed()` and the boot exposure warning. `src/server/index.ts` — the single `http.createServer` that picks engine handler vs Express per request, and the `cors` middleware that consults it.
...
tiles (resident workers against the `maxWarmWorkers` ceiling, **lambdas loaded against lambdas
declared**, peak parallelism, host memory %, load normalised by core count), a step area of
parallelism over the window, and a timeline with one row **per lambda** and one bar per invocation —
overlapping bars in a vertical slice *are* the parallelism. Under it, each live worker lists the
handlers actually inside it: a worker is one process serving every function of its service, each
loaded on its own first invocation, so "the service is warm" answers a different question from
"which lambda is up" — with 1024 registered functions and 20 in use, the second one is the question.
...
**Where it lives.** `src/ui/src/components/ActivityPanel.vue` — the panel.
...
counters. `src/server/services/lambda-runtime-manager.ts` — records one span per invocation and
mirrors each worker's loaded-handler set. `src/server/runtime/runtime-worker.ts` — reports the
handler it loaded with every result, since the module cache is the worker's own state.
...
**How it works.** The list refreshes every 10s with each service's status, path, resource breakdown and last-updated time; a path box registers a new one via `POST /api/services/register`. Start/stop drive the orchestrator's process control: `POST /api/services/:name/start` spawns the service's own npm start script (`npm run start`, or `start:<stage>`) in the root recorded in its cache entry — the browser sends only the service name, and the endpoint accepts no `args`, `cwd` or `env`, so what runs is decided entirely by the server. A logs modal tails `GET /api/services/:name/logs` every 2s while it is open. The detail page groups the service's provisioned resources by type (lambda, dynamodb, sqs, sns, s3, eventbus, event-rule, opensearch, event-source), refreshes every 10s, and offers the same start/stop/logs plus a confirm-guarded delete.
...
<details>
<summary><b>Service wiring diagram</b> — the service detail screen draws what talks to what, and says how it knows</summary>

**How it works.** `GET /api/services/:name/graph` derives the diagram from the packaged CloudFormation template the registrar cached plus that service's registration metadata — never from the running engine, so a stopped service answers exactly like a running one and no arrow disappears because a worker went cold. Nodes are the service's declared resources (Lambda, DynamoDB, SQS, SNS, S3, EventBridge bus and rule, OpenSearch, Secrets Manager), one node **per HTTP route** (`POST /orders`, not a single API box — which function serves which path is the first thing this screen is for), the execution role when more than one function shares it, and an `external` node for every ARN or name the service references but does not declare, which is where the cross-service links surface. Every edge carries a kind and a **confidence**. `declared` means CloudFormation (or `serverless-state.json`) wires it and the provisioner acts on it: HTTP route → function, authorizer → route, event-source mapping (SQS poll or DynamoDB stream, with batch size), S3 notification (with the event names), bus → rule → target, SQS redrive (with `maxReceiveCount`), SNS subscription (with the protocol) and IAM policy grant (with the granted actions). `inferred` covers exactly one case — an environment variable whose value names a declared resource, matched as a whole name, as an ARN or as the last segment of a service URL, **never** as a substring — and those edges are additionally flagged `serviceWide` when the identical key/value sits on every function of the service, i.e. it came from `provider.environment`, was copied onto each function by the packager and therefore says nothing about the one it is drawn on. IAM is the other place the drawing is deliberately not literal: the framework emits **one** execution role shared by every function, so a grant on it means "every Lambda bound to this role may touch that table" — with one function on the role the edge is drawn straight to the resource, with several the role becomes a node and the grants read as N + M edges through a hub instead of an N × M fan-out (at ≈60 functions × ≈10 tables the product form is 600 arrows of noise, and the hub is what the template literally says). Framework noise is dropped on purpose — `Resource: "*"` grants, `logs:`/`xray:`/`sts:` actions, the custom-resources helper role, the API Gateway v2 plumbing resources and endpoint-shaped env vars like `AWS_ENDPOINT` — and an IAM statement that merely restates an event-source mapping lands on the existing arrow instead of doubling it. Anything that could not be resolved comes back in `warnings[]` rather than vanishing. The endpoint answers **400** for an invalid name, **404** for an unknown service, and an empty graph carrying one warning when the service is registered but has no cached template. The same payload is on `client.services.graph(name)` and the `lss_service_graph` MCP tool.

**Where it lives.** `src/server/services/service-graph.ts` — `buildServiceGraph()`, pure and I/O-free, plus `parseResourceArn()` and the node/edge types. `src/server/routes/services.ts` — `GET /:name/graph`. `src/ui/src/components/graph/` — the layout and the canvas drawing, hosted by `src/ui/src/pages/ServiceDetailPage.vue`. `src/ui/src/services/api.ts` — the typed call. `src/client/namespaces/services.ts` (`services.graph(name)`) and `src/mcp/tools.ts` (`lss_service_graph`) — the same graph for a test or an agent.

**Why it exists.** The detail page already answered "what does this service own"; "and what is wired to what" still meant reading a packaged template by hand — which is exactly where the interesting arrows hide, since a rule listening on another service's bus or a grant on someone else's queue is the reason a monorepo runs one stack instead of fifteen. The confidence flag is the payload's point rather than a footnote: an env-var name match is a guess, and a diagram that draws a guess like a declared wire is worse than no diagram at all.
...
**How it works.** `ConfigManager` is a singleton that searches, in order: `$LSS_CONFIG_PATH` (exported by the CLI from `lss <cmd> --config <path>`), `./lss.config.json`, `./.lssrc`, `~/lss.config.json`, `~/.lssrc`. The first file that exists is parsed and wins; a file that fails to parse logs a warning and the search continues to the next candidate. Environment variables are then applied *over* the file — `LSS_DASHBOARD_PORT`/`PORT`, `AWS_REGION`, `LSS_PERSISTENCE`, `LSS_DEBUG`, `LSS_SEEDS_DIR`, `LSS_ENABLE_DYNAMO_PROXY`, `LSS_DYNAMO_PROXY_PORT`, `LSS_AUTO_PACKAGE`, `LSS_PACKAGE_COMMAND`, `LSS_PACKAGE_TIMEOUT_MS`, `LSS_LAMBDA_RUNTIME`, `LSS_LAMBDA_EXECUTION`, `LSS_LAMBDA_WATCH`, `LSS_INVOKE_HOST`, `LSS_ENGINE_PORT`, `LSS_ENGINE_DATA_DIR`. Every key is optional; the resolved defaults are `serverPort` 14566, `selfEngine.port` 14566 (same value, which is what puts the API, the dashboard and the AWS wire on **one** listener — give them different values to split them), `region` `us-east-1`, `persistence` true, `debug` false, `seedsDir` `./seeds`, `dynamoProxyPort` 8000, `autoPackage` false, `packageCommand` `npx serverless package`, `packageTimeoutMs` 300000, `selfEngine.account` `000000000000`, `idleUnloadMs` 300000, `memoryBudgetMb` 128, `fsync` false, `fallbackEndpoint` null. There is no `engine` key: 1.0 dropped it along with the LocalStack backend, so a leftover 0.x `engine: "localstack"` is now just an unknown key — ignored on load, and rejected by `PUT /api/config` as `unknown config key` like any other. Two settings deliberately sit **outside** this layer: `LSS_BIND_HOST` (default `127.0.0.1`) and `LSS_CORS_ORIGINS` (default: loopback origins) are read directly from the environment by `src/server/services/bind-host.ts` and have no config key and no `PUT /api/config` spelling, because the pair of values that decides who can reach an unauthenticated API must not be editable *through* that API. Three settings that stay inside it are validated when they arrive through `PUT /api/config`: `packageCommand` (global and per-service) must match the packaging grammar — an allowed runner *and* an allowed subcommand, `npm|yarn|pnpm run …`, `serverless|sls|osls package …` or `npx` launching one of those CLIs — while `packageArgs` and the command's own tokens are screened for flags that re-point the runner (`--node-options`, `--script-shell`, `--prefix`, …), and `packageEnv` must carry no key that injects code into a child process (`NODE_OPTIONS`, `LD_PRELOAD`, `DYLD_*`, …). All three end up as arguments to `spawn()`, so "any non-empty string" was never a validation on an unauthenticated route — and neither was checking the command's first token, since `npm exec -c '<shell>'` passes that check.
...
**How it works.** `PUT /api/config` takes a partial patch and validates every key against a spec table (`port` = integer 1–65535, `positiveInt`, `nonNegativeInt`, `boolean`, `string` with optional enum, `packageCommand`, `packageArgs`, `stringRecord`, `envRecord`, `object` with fixed subkeys or per-entry subkeys); unknown keys and unknown subkeys are rejected, and **all** errors come back at once as a 400 with a `details[]` array. Three keys get more than a type check because they become arguments to `spawn()` and this route is unauthenticated: `packageCommand` (global and every `servicePackaging[*]` override) must match the packaging grammar — runner **and** subcommand, so `npm exec -c '<shell>'` and `yarn dlx <pkg>` are rejected alongside `/bin/sh`; `packageArgs` is screened for runner-redirecting flags because it reaches the same argv without a tokenizer; and `packageEnv` must carry no code-injection key (`NODE_OPTIONS`, `LD_PRELOAD`, `DYLD_*`, …). `secrets` is explicitly blocked — seed material must never cross the dashboard API. The write re-reads the raw file from disk and patches it key-by-key (never the env-resolved in-memory config, so env values are not baked in); object blocks merge one level deep, map blocks like `serviceRuntime` two levels, and a `null` deletes a key or subkey. **`packageEnv` — global and every `servicePackaging[*]` copy — merges one level deeper still, per variable**: `{"packageEnv":{"A":"v"}}` sets only `A` and leaves `B` and `C` alone, `{"packageEnv":{"A":null}}` removes only `A`, and `{"packageEnv":null}` still drops the whole block (a map left empty by a removal is dropped too, rather than written as `{}`). It is the one editable key whose values are **write-only** — `GET /api/config` reports `packageEnvKeys`, the names alone — so a dashboard that never received `B` and `C` has nothing to resend for them, and a replace-the-map write would have deleted them on the first save. Per-variable is what makes editing packaging env vars from the UI possible at all. The loader-hook denylist is unchanged for every variable a patch **sets**; *removing* one is always allowed, since it only takes capability away and is the one way to undo a hand-edited `NODE_OPTIONS` from the dashboard (the fence guards this API, never the file). The manager then reloads and answers with `restartRequired` — the diff of boot-materialized values (`serverPort`, `enableDynamoProxy`, `dynamoProxyPort`, `region`, `persistence`, `debug`, `stateDir`, and the `selfEngine` block), which keep their old value until `lss stop && lss start` — plus `envOverridden`, the patched keys currently masked by an env var. `POST /api/config/reload` runs the same reload after a hand edit; if the loaded file no longer parses it answers 400 and keeps the working config. `GET /api/config` returns the resolved snapshot with `packageEnv` collapsed to key names and secret seeds to a count, and `GET /api/config/ports` lists every port the stack exposes (orchestrator, engine, DynamoDB proxy, each service's API and invoke listener).
...
npx lss register --repackage   # ...building them first, instead of registering the artifacts on disk
...
Every listener LSS opens binds **`127.0.0.1`** by default, and the API grants CORS to
loopback origins only. This API has no authentication and every table, queue, bucket and
secret in the stack is readable through it, so it is not offered to the network unless
you ask. Asking is one line:

```bash
LSS_BIND_HOST=0.0.0.0 LSS_CORS_ORIGINS='*' npx lss start   # trusted networks only
```

That is the **container recipe**: LSS inside a container, browser and your own frontends
on the host. `LSS_BIND_HOST` is what makes a published port work at all — with
`docker run -p 14566:14566` the connection arrives on the container's *external*
interface, which a loopback socket refuses — and it covers **every** listener at once:
the dashboard/REST/AWS port, each service's API and Lambda-invoke port, the
split-listener engine, the DynamoDB proxy. `LSS_CORS_ORIGINS` is the browser half; a
frontend served from anything other than `localhost` needs it, and naming your origins
beats the wildcard when you know them:
...
Both are environment variables and nothing else — no `lss.config.json` key, no Settings
field — so the exposure cannot be widened *through* the API it protects. VS Code
devcontainer forwarding needs neither: it attaches from inside the container, so the
default bind is enough and the browser sees `http://localhost:<port>`. What you accept
by widening is an unauthenticated API on the network whose callers can read and write
your local emulator data and read secret values. That is bounded rather than a shell on
the host — `/start`, `/install` and `/package` take no caller-chosen binary — but it is
not zero, so use it on networks you trust.
Boot prints a warning naming exactly what you opened. Full detail:
[docs/CONFIGURATION.md](docs/CONFIGURATION.md#lss_bind_host-network-exposure).
...
per-service resource breakdown incl. EventBridge buses & rules, plus a wiring diagram
of what the service declared talking to what — declared wires and guessed env-var
matches drawn apart), **Lambdas** (registry + invoke), **APIs** (emulated HTTP
routes), **Queues** (send/receive/purge, consumers),
...
- **Nothing answers from another machine, a phone, or a published Docker port** → that is the default: every listener binds `127.0.0.1`. `LSS_BIND_HOST=0.0.0.0` opens all of them at once (dashboard/REST/AWS, per-service API and invoke ports, split-listener engine, DynamoDB proxy) — the API has no authentication, so do it on a network you trust. Docker still has to `-p` each port you want forwarded. (A VS Code devcontainer needs none of this; its forwarder attaches from inside the container.)
- **The browser console says the request was blocked by CORS** → with `LSS_CORS_ORIGINS` unset the API answers only loopback origins (`http://localhost`, `http://127.0.0.1`, `http://[::1]`, any port). If the blocked page is *yours*, list it: `LSS_CORS_ORIGINS=http://192.168.1.20:5173` (comma-separated, exact origins, or `*` for any) — copy the origin verbatim from the console error, since scheme and port both count. Setting it replaces the loopback default, so include your loopback origins too. `curl`, the `LssClient` and the MCP tools send no `Origin` and were never affected. A blocked call to a **service's** 30xx port is different — that gateway answers from the route's own `cors:` declaration, as AWS does.
