...
|---|---|---|
| `lss start` | Starts the orchestrator + engine in the background on **one port** (default 14566: dashboard, REST API and AWS wire); writes a PID file and logs. **Exits non-zero when the boot dies**, printing the log tail and who holds the port. | integration (`features.test.ts` boots via `lss start --config`) + unit (`cli`) |
| `lss start --wait[=<seconds>]` | Blocks until `GET /api/health` answers (default 30 s), then exits 0; on timeout or a dead child, prints the log tail and exits 1 — so a sequential chain never runs `register` against a stack that never came up. | unit (`cli`) |
| `lss stop` | Gracefully stops the orchestrator addressed by the active config. | integration (`features.test.ts` teardown) + unit (`cli`) |
| `lss status` | Reports RUNNING/NOT RUNNING + ports for the addressed instance. When this project has no PID file it asks `GET /api/config` who is on the port and names the owning `projectRoot` — under `--net=host` the port is machine-wide, and "NOT RUNNING" about a port answering 200 points at the wrong place. | unit (`cli`) |
...
| EventBridge buses & rules | `AWS::Events::EventBus` created on the engine; `AWS::Events::Rule` (pattern or schedule) wired to Lambda targets; `AWS::Events::Archive` skipped with a warning (never listed or replayable). A rule whose **custom bus does not exist yet** is parked with a warning and provisioned automatically when the stack that owns the bus registers — registration order is whatever the operator typed, so a consumer registered first must not end up silently mute. | unit (`cloudformation-parser`, `resource-provisioner`) |
...
| Feature | Promise | Asserted by |
|---|---|---|
| Registration API | `POST /api/services/register` with a bare `{ servicePath }` is complete: the orchestrator packages when the template is missing (`autoPackage`) and reads name/region/`custom.lss` ports from the packaged `serverless-state.json`. Explicit `apiPort`/`invokePort`/`region` in the payload win over the state hints; `serviceRuntime` (config) wins over both. Optional `repackage: true` forces the packaging command to run first even when a template is present; a non-boolean is a 400 rather than a coercion. | unit (`routes/services`, `serverless-state-parser`) + integration |
| Registration warnings | The response's `warnings[]` carries **everything that did not happen**: each resource provisioning could not create, each rule waiting for a bus, and a service that declares HTTP routes but resolved no `apiPort` (it registers, serves nothing over HTTP, and now says so). Provisioning stays non-fatal — non-fatal is not the same as silent. | unit (`resource-provisioner`, `service-registrar`) |
| Port provenance | The layers below `serviceRuntime` (request payload, then `custom.lss`) are stored as `portHints`, and the config layer is re-applied on every activation — so an edited `serviceRuntime` port applies on the next boot and a **deleted** one falls back to the service's own hint instead of the cached number outliving the config. | unit (`service-registrar`) |
| Service discovery | `GET /api/services/scan` / `lss scan` walk the project root (depth ≤ 6, dependency/build/VCS trees skipped, a service root is a leaf) and report every Serverless/osls service with `installed`/`packaged`/`registered` flags plus effective ports and package command (`serviceRuntime`/`servicePackaging` overlays win over yml hints — same precedence registration applies). `scanIgnore` drops the stacks a monorepo never registers locally; the "not packaged" warning is worded from the **effective** `autoPackage`; `hasFunctions` marks a resources-only stack so onboarding stops offering it ports. | unit (`service-scanner`, `routes/services`, `cli`) |
...
| CLI registration | `lss register [path...]` (defaults to `.`) POSTs each path and exits non-zero if any fails; `lss scan` prints the checklist. `--repackage` builds before registering. | unit (`cli`) + integration (live proof) |
| Stale-artifact warning and `--repackage` | `autoPackage` packages only when the template is **missing**, so registering an already-packaged service re-registers whatever is on disk — possibly many commits old — and answers `✓` in ~0.3 s. Registering is the natural gesture after editing code, so that `✓` was read as "my code is loaded" when it could mean "yesterday's build is loaded": the registration was faithful, the silence was the defect. Registration now takes the **newest** mtime among `.serverless/` artifacts (newest, so a leftover zip from an older run cannot fake staleness) and short-circuits on the first source file past it, walking the service root minus `node_modules`, `dist`, `build`, `coverage` and every dot-directory — which is also what keeps the walk cheap at 40 services. A positive verdict pushes a warning into the same `warnings[]` the CLI already prints, naming the file that moved, both timestamps and **that service's own** effective package command (the datum `lss.config.json` holds and the operator otherwise had to know from outside). It never blocks: what was packaged still registers. `lss register --repackage` / `{ repackage: true }` runs the packaging command first even with a template present, and does so regardless of `autoPackage` — an explicit gesture is not the automatic path it governs. An undecidable case (no `.serverless/`, nothing readable in it, an unreadable directory) warns nothing rather than guessing. | unit (`artifact-staleness`, `routes/services`, `cli`) |
| Guided onboarding | First dashboard visit with no services opens a 3-step flow — ports, branding, project scan with tick-to-select and install → package → register buttons; per-service ports and package commands are editable inline and persist to `lss.config.json` (`serviceRuntime`/`servicePackaging`, merged per entry so sibling settings survive). A resources-only stack is offered no ports at all, and a port edit that only appends a digit to the current value (`3072` → `30729`) has to be confirmed before it is written — that exact slip put three services on dead ports, silently. Reopenable from Settings. | vue-tsc + manual |
| Service wiring graph | `GET /api/services/:name/graph` answers what the service declared talking to what, derived from the packaged CloudFormation template plus that service's registration metadata — **never** from the live engine, so a stopped service answers exactly like a running one. Nodes: every declared resource, one **per HTTP route** (not one API box), the execution role when several functions share it, and an `external` node for every ARN or name referenced but not declared — the cross-service links a per-service view would otherwise hide. Every edge carries a kind and a **confidence**. `declared` is what CloudFormation or `serverless-state.json` actually wires and the provisioner acts on: route → function, authorizer → route, event-source mapping (with batch size), S3 notification (with the event names), bus → rule → target, SQS redrive (with `maxReceiveCount`), SNS subscription (with the protocol), IAM policy grant (with the granted actions). `inferred` is the single guessed kind — an environment variable whose value names a declared resource by whole name, by ARN or by the last segment of a service URL, never by substring — and carries `serviceWide` when the identical key/value sits on every function, i.e. it came from `provider.environment` and says nothing about the one function it is drawn on. IAM grants on a **shared** role are drawn through a role node (N + M edges) instead of an N × M fan-out, which at ≈60 functions × ≈10 tables is the difference between 70 arrows and 600; `Resource: "*"`, `logs:`/`xray:`/`sts:` actions, the framework's custom-resources helper role and endpoint-shaped env keys are dropped as noise, and an IAM statement restating an event-source mapping lands on the existing arrow rather than doubling it. Unresolvable references surface in `warnings[]` instead of vanishing. 400 on an invalid name, 404 on an unknown service, an empty graph plus one warning when the service is registered but nothing is cached. Backs the service detail screen's wiring canvas, `client.services.graph(name)` and the `lss_service_graph` MCP tool. | unit (`services/service-graph`, `routes/services`, `client/http`, `mcp/tools`) |
...
collections, and the detail page draws the service's **wiring diagram** on a canvas from
`GET /api/services/:name/graph` (§4) — declared wires and inferred env-var name matches drawn apart, IAM
grants hubbed through a shared-role node (UI is exercised manually, not in the automated suites). The
Overview shows an **Exposed ports** card (`GET /api/config/ports`); the Settings tab edits
`lss.config.json` via `PUT /api/config` (dirty fields only, restart-required and env-masked keys flagged)
and re-reads it via `POST /api/config/reload`.
...
counters, declared function count **and `loadedFunctions` — which handlers are actually in that worker's
module cache**), the residency policy (`warm`/`maxWarmWorkers`/`lazy`/`idleTimeoutMs` plus
`loadedFunctions`/`registeredFunctions`) and host counters (orchestrator RSS, free/total RAM, CPU count,
1-minute load, uptime). Spans come from a stack-wide, log-free ring capped at 1000 entries
(`invocation-activity.ts`), recorded on every invocation; a span still running when the window opens is
included, so a long handler reads as load rather than silence.

**Processes and handlers are different units, and the panel says both.** A worker is one OS process serving
every function of its service, each loaded on ITS first invocation — so "billing-service is warm" does not say
whether that is one handler or sixty, nor which. The worker reports the function it loaded on every result
(`loadedFunction`), the manager mirrors that per runtime and clears it whenever the process dies (stop, idle
unload, crash, re-registration), and the dashboard renders stat tiles + a step area + a **per-lambda** span
timeline + the handlers resident inside each live worker. That is the difference between "which service has a
process" and "which lambda is up", which is the question being asked with 1024 registered functions and 20 in
use. Asserted by: unit (`invocation-activity`, `routes/lambdas`, `runtime/runtime-worker`).
...
(Claude Code included) as **26 tools**, so an AI agent drives the stack directly instead of being handed
`curl` output: health/config/ports, services, resources and owners, **one service's wiring graph**, lambdas +
per-invocation logs, invoke, HTTP routes, DynamoDB tables/scan/query/put, queues + send + **await-idle**,
buckets + object listings, secrets (names only, never values), OpenSearch search, and seeds.
